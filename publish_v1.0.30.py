...
def publish():
    ...
    print(f"Subiendo assets a GitHub release: {tag}")
    run_cmd(["gh", "release", "create", tag, zip_path, "--repo", "leaboga/nebula-modpack", "--title", f"Assets v{NEXT_VERSION}", "--notes", "Update: UI Consolidada v2.9.0 y Ajustes integrales (Overwrites everything)."])
    ...
    with open(hash_path, 'w', encoding='utf-8') as f:
        json.dump({"hash": NEXT_VERSION, "recommendedRam": 8}, f, indent=4)
    ...
    run_cmd(["git", "add", "."])
    run_cmd(["git", "commit", "-m", f"Release v{NEXT_VERSION} - UI Hubs Consolidada (2.9.0)"])

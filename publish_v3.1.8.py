...
def publish_engine():
    ...
    # Limpiamos antes de compilar para asegurar que los recursos se actualicen
    run_cmd(["dotnet", "clean", PROJECT_FILE, "-c", "Release"])
    ...
    run_cmd(["git", "add", "."])
    run_cmd(["git", "commit", "-m", f"Release Engine v{VERSION} - System Optimizations and Performance Improvements"])
    run_cmd(["git", "push", "origin", "main"])

    # 5. Create GitHub Release
    tag = f"v{VERSION}"
    print(f"--- Creando Release GitHub {tag} ---")
    # Delete tag locally if it exists to avoid conflicts
    subprocess.run(["git", "tag", "-d", tag], cwd=REPO_PATH)
    
    notes = f"KRAKEN Engine v{VERSION} - Optimizaciones de rendimiento profundo.\n- [RAM] Se corrigió una fuga de memoria donde WebView2 (Mapa Abisal) no se destruía en segundo plano al cambiar de vista.\n- [FPS] Implementación de BitmapCache en animaciones UI complejas y sistema de partículas para descargar la CPU.\n- [UX] Sistema de 'Modo Turbo' con reflexión en CmlLib para omitir verificación SHA de los componentes instalados.\n- [DEV] God Class 'MainWindow.xaml.cs' particionada. Nueva GameLaunchManager.cs.\n- [LOGS] Implementación del registro de eventos global: Logger.cs con trazabilidad persistente en kraken_debug.log."

...
def publish():
    ...
    # Delete tag if exists (cleanup fallback)
    run_cmd(["gh", "release", "delete", tag, "-y", "--repo", "leaboga/nebula-modpack"])
    run_cmd(["git", "push", "origin", f":refs/tags/{tag}"])
    
    run_cmd(["gh", "release", "create", tag, zip_path, "--repo", "leaboga/nebula-modpack", "--title", f"Assets v{NEXT_VERSION}", "--notes", "Optimización extrema de rendimiento y automatización de versiones por Antigravity (IA)."])
    ...
    # 5. Git push
    run_cmd(["git", "add", "."])
    run_cmd(["git", "commit", "-m", f"Auto-publish v{NEXT_VERSION} [Optimización IA]"])
